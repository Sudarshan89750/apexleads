...
import {
  PlayCircle, Mail, Tag, Clock, GitFork, MessageSquare, CalendarPlus, TagIcon, PlusCircle, Edit, Trash2
} from 'lucide-react';
...
export const ActionNode: React.FC<NodeProps<WorkflowAction>> = ({ data, id }) => {
  const openDialog = useAutomationBuilderStore((s) => s.openDialog);
  const isBranching = data.type === 'if_else';
  return (
    <Card className="w-80 shadow-md group">
      <CardHeader>
        <CardTitle className="flex items-center justify-between text-base">
          ...
          <div className="opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1">
            <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => openDialog({ type: 'edit', action: data, nodeId: id })}>
...
export const PlaceholderNode: React.FC<NodeProps<{ label: string; parentNodeId: string; sourceHandle?: string }>> = ({ data }) => {
  const openDialog = useAutomationBuilderStore((s) => s.openDialog);
  return (
    <>
      <div
